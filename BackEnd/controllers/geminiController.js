...
Returns: 7 days unopened. Delivery: 2-3 days.`;
  ...
  if (userRole === "admin" || userRole === "superAdmin")
    base += " Can answer sales, inventory and user questions.";
  return base;
}
  ...
  try {
    ...
    if (needsProducts) {
      const products = await Product.find({ isActive: true })
        .select("name price stock variety")
        .limit(15)
        .lean();
      if (products.length)
    ...
    if (needsOrders) {
      const orders = await Order.find({ user: userId })
        .sort({ createdAt: -1 })
        .limit(3)
        .select("_id orderStatus paymentStatus totalAmount")
        .lean();
      if (orders.length)
...
const chat = async (req, res) => {
  try {
    ...
    const [contextData, history] = await Promise.all([
      buildContextData(message, userId, userRole),
    ...
    try {
      ...
      if (fetchErr.name === "AbortError")
        return res
          ...
          .json({
            error:
              "Ollama is taking too long. Make sure Ollama is running with: ollama serve",
          });
      throw fetchErr;
    }
    clearTimeout(timeoutId);
    ...
    if (!ollamaRes.ok) {
      ...
      return res
        .status(502)
        .json({
          error: `Ollama error (${ollamaRes.status}): ${errText || "Make sure Ollama is running"}`,
        });
    }
    ...
    if (!aiReply)
      return res
        .status(502)
        .json({ error: "Empty response from Ollama. Please try again." });
